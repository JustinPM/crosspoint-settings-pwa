...
<head>
    ...
    <style>
        ...
        .log-stream { background: #000; color: #008f11; padding: 12px; font-size: 11px; height: 100px; overflow-y: auto; border: 1px solid #222; margin-bottom: 20px; font-family: 'Consolas', monospace; }
...
<div class="container">
    <div class="card">
        ...
        <div id="terminal" class="log-stream">READY_FOR_COMMANDS...</div>
    </div>

    <div class="card">
        <h2>⚙️ READER SETTINGS (settings.bin)</h2>
        <input type="file" id="setIn" onchange="readSettingsFile()" style="margin-bottom:20px;">
    ...
    <div class="card">
        ...
        <input type="file" id="wifiIn" onchange="readWifiFile()" style="margin-bottom:15px;">
...
<script>
    ...
    async function readSettingsFile() {
        try {
            ...
            for(let i=0; i<15; i++) { if (ptr < data.length) document.getElementById(`field_${i}`).value = data[ptr++]; }
            ...
            ptr += sLen;
            for(let i=15; i<18; i++) { if (ptr < data.length) document.getElementById(`field_${i}`).value = data[ptr++]; }
        } catch(e) { print("Error reading settings file."); }
    }
    ...
    async function readWifiFile() {
        ...
        } catch(e) { print("Error reading wifi file."); }
    }
    ...
    async function uploadFile(type) {
        ...
        try {
            ...
            print(`TRANSMITTED: Check device.`);
        } catch (err) { print(`ERROR: Request failed.`); }
    }

    // PWA Logic: Registers the Service Worker to make it installable
